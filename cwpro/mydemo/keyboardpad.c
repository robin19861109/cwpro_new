...
void emwin_main(void) 
{
 ...
 //   BUTTON_SetDefaultSkin(_DrawSkinFlex_BUTTON); /* 设置BUTTON的皮肤 */
    ...
    /* 创建FRAME窗口 */
    hFrame = FRAMEWIN_CreateEx(0, 0, 240, 160, WM_HBKWIN, WM_CF_SHOW, 0, 0, "Setting Pwd For Image", 0);
    ...
    /* 创建edit(文本小工具)小工具 */
		 hEdit = EDIT_CreateEx(32, 47, 154, 22, WM_GetClientWindow(hFrame), WM_CF_SHOW, GUI_ID_EDIT0, 100, 100);
     EDIT_EnableBlink(hEdit, 500, 1); /* 开启光标,周期500ms */
		 EDIT_SetInsertMode(hEdit, 1);     /* 开启插入模式 */
		 EDIT_SetFont(hEdit, GUI_FONT_8X16_1);
		 WM_SetFocus(hEdit);
		 
		 
		 hEdit = TEXT_CreateEx(5, 13, 189, 21, WM_GetClientWindow(hFrame),WM_CF_SHOW,GUI_ID_TEXT0,2,0);
  //   TEXT_SetText(hEdit, "Enter your password to login.");
	
	//Please check your password and try again
	
	TEXT_SetText(hEdit, "Check your password and try again.");
     TEXT_SetFont(hEdit, GUI_FONT_13HB_ASCII);
	
     hEdit = BUTTON_CreateEx(64, 88, 80, 20, WM_GetClientWindow(hFrame),WM_CF_SHOW,GUI_ID_BUTTON0,2);
     BUTTON_SetText(hEdit, "Next");
     BUTTON_SetFont(hEdit, GUI_FONT_16B_1);

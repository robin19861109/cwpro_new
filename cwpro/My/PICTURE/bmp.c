...
/**
 * @brief       标准的bmp解码,解码filename这个BMP文件
 *   @note      可以解码大尺寸BMP(自动缩放), 不过速度比较慢
 * @param       filename : 包含路径的文件名(.bmp)
 * @retval      操作结果
 *   @arg       0   , 成功
 *   @arg       其他, 错误码
 */
...
{    FIL *f_bmp;
    ...
    databuf = (uint8_t *)piclib_mem_malloc(readlen); /* 开辟readlen字节的内存区域 */
    ...
    if (res == 0)   /* 打开成功 */
    {
        ...
        count = databuf[0x0e]+databuf[0x0f]*256+databuf[0x10]*256*256+databuf[0x11]*256*256*256;//(int)pbmp->bmfHeader.bfOffBits;              /* 数据偏移,得到数据段的开始地址 */
			  printf("ssssssssssssssssssssssss\r\n");
			  printf("count = %d   pbmp->bmfHeader.bfOffBits = %4x\r\n",count , pbmp->bmfHeader.bfOffBits);
			
			
        color_byte = databuf[0x1c] /8;//pbmp->bmiHeader.biBitCount / 8;    /* 彩色位 16/24/32 */
			
			printf("color_byte = %x  pbmp->bmiHeader.biBitCount=%x \r\n",color_byte , pbmp->bmiHeader.biBitCount);
        biCompression = databuf[0x1e];//pbmp->bmiHeader.biCompression;  /* 压缩方式 */
        picinfo.ImgHeight = databuf[0x16]+databuf[0x17]*256+databuf[0x18]*256*256+databuf[0x19]*256*256*256;//pbmp->bmiHeader.biHeight;   /* 得到图片高度 */
        picinfo.ImgWidth = databuf[0x12]+databuf[0x13]*256+databuf[0x14]*256*256+databuf[0x15]*256*256*256;//pbmp->bmiHeader.biWidth;     /* 得到图片宽度 */
            ...
            while (count < readlen)     /* 读取一簇1024扇区 (SectorsPerClust 每簇扇区数) */
            {
                ...
                if (rgb == color_byte)  /* 水平方向读取到1像素数数据后显示 */
                {
                    if (x < picinfo.ImgWidth)
                    {
                        ...
                        if (piclib_is_element_ok(realx, realy, 1) && yok)   /* 符合条件 */
                        {
                ...
                if (countpix >= rowlen) /* 水平方向像素值到了.换行 */
                {
                    ...
                    if (piclib_is_element_ok(realx, realy, 0))yok = 1; /* 此处不改变picinfo.staticx,y的值 */
                    else yok = 0;
 ...
 *   @note      本函数仅支持小于LCD屏幕分辨率的bmp文件解码.
 * @param       filename    : 包含路径的文件名(.bmp)
 * @param       x, y        : 显示起始坐标
 * @param       width,height: 显示区域大小
 * @param       acolor      : 附加的alphablend的颜色(这个仅对32位色bmp有效!!!)
 * @param       mode        : 模式(除了bit5,其他的均只对32位色bmp有效!!!)
 *                              bit[7:6]: 00, 仅使用图片本身和底色alphablend;
 *                                        01, 仅图片和acolor进行alphablend,并且不适用附加的透明度; 
 *                                        10, 底色,acolor,图片,一起进行alphablend;
 *                              bit5    : 保留
...
#if BMP_USE_MALLOC == 1                 /* 使用malloc */
    databuf = (uint8_t *)malloc(readlen); /* 开辟readlen字节的内存区域 */

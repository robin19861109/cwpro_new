    ...
    if (hColor && hBkGnd) {
      ...
      for (y = 0; y < Height; y++) {
        ...
        if (HasAlpha || HasTrans) {
          ...
          if (xSize > 0) {
            GUI_ReadRectEx(xPos, y0 + y, xPos + xSize - 1, y0 + y, p, GUI_Context.apDevice[GUI_Context.SelLayer]);
            GUI__ExpandPixelIndices(p, xSize, GUI_GetBitsPerPixelEx(GUI_Context.SelLayer));
          }
        }
        #if (GUI_WINSUPPORT)
          WM_Activate();
          GUI_Context.ClipRect = ClipRect;
        #endif
        if (HasAlpha) {
          //
          // Mix with background

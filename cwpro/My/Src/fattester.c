...
{
    fattester.file = (FIL *)Mem_malloc(0, sizeof(FIL));      /* 为file申请内存 */
    fattester.fatbuf = (uint8_t *)Mem_malloc(0, 512);        /* 为fattester.fatbuf申请内存 */
...
void mf_free(void)
{
    Mem_free(0, fattester.file);
    Mem_free(0, fattester.fatbuf);
}

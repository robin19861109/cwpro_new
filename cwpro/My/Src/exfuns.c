...
uint8_t exfuns_init(void)
{
    ...
    {
        fs[i] = (FATFS *)Mem_malloc(0, sizeof(FATFS));   /* 为磁盘i工作区申请内存 */
 ...
 */
uint8_t exfuns_file_type1(char *fname,const char* type)
{
    uint8_t tbuf[5];
    char *attr = 0;   /* 后缀名 */
    uint8_t i = 0, j;
    ...
    for (i = 0; i < 4; i++)tbuf[i] = exfuns_char_upper(tbuf[i]);    /* 全部变为大写 */
            ...
            if (strcmp((const char *)type, (const char *)tbuf) == 0) /* 找到了 */
            {
                return 1;
            }
                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    fsrc = (FIL *)Mem_malloc(0, sizeof(FIL));    /* 申请内存 */
    fdst = (FIL *)Mem_malloc(0, sizeof(FIL));
    fbuf = (uint8_t *)Mem_malloc(0, 8192);
...
#define MAX_PATHNAME_DEPTH  512 + 1     /* 最大目标文件路径+文件名深度 */
    ...
    fddir = (DIR *)Mem_malloc(0, sizeof(DIR));   /* 申请内存 */
    finfo = (FILINFO *)Mem_malloc(0, sizeof(FILINFO));

    if (fddir == NULL || finfo == NULL)res = 100;

    if (res == 0)
    {
        pathname = Mem_malloc(0, MAX_PATHNAME_DEPTH);
...
#define MAX_PATHNAME_DEPTH 512 + 1  /* 最大目标文件路径+文件名深度 */
    ...
    srcdir = (DIR *)Mem_malloc(0, sizeof(DIR));  /* 申请内存 */
    dstdir = (DIR *)Mem_malloc(0, sizeof(DIR));
    finfo = (FILINFO *)Mem_malloc(0, sizeof(FILINFO));
    ...
    if (res == 0)
    {
        dstpathname = Mem_malloc(0, MAX_PATHNAME_DEPTH);
        srcpathname = Mem_malloc(0, MAX_PATHNAME_DEPTH);
        ...
        if (res == 0)
        {
            ...
            Mem_free(0, dstpathname);
            Mem_free(0, srcpathname);
        }
    }

...
//定义LCD的命令和数据位.
uint32_t UCGUI_LCD_CMD  = 0X6C001FFE; //地址为0X6C001FFE; 
uint32_t UCGUI_LCD_DATA = 0X6C002000; //地址为0X6C002000;  
...
/*static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
	...
	uint32_t n;
		...
		//设置窗口大小,X轴坐标的起始和终止坐标
		*(__IO uint16_t *)(UCGUI_LCD_CMD)  = setxcmd; 
		...
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (x1&0XFF);
		...
		//设置窗口大小,Y轴的起始和终止坐标
		*(__IO uint16_t *)(UCGUI_LCD_CMD)  = setycmd; 
		...
		*(__IO uint16_t *)(UCGUI_LCD_CMD)  = write_gramcmd;  //写入颜色值
		while(n--)
		{
		...
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (lcd_width>>8);  
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (lcd_width&0XFF);
		
		//恢复窗口大小
		*(__IO uint16_t *)(UCGUI_LCD_CMD)  = setycmd; 
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (0X00>>8);
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (0X00&0XFF);
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (lcd_height>>8); 
		*(__IO uint16_t *)(UCGUI_LCD_DATA) = (lcd_height&0XFF);
